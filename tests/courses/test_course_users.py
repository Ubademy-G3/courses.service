...
test_request_payload = {"user_id": str(global_user_id), "user_type": "student", "progress": 0.0, "approval_state": False}
...
class CourseUserTest(TestCase):
    ...
    def test_add_course_user(self, mock_method_post, mock_get, mock_course):
        ...
        response = test_app.post(
            "/courses/" + str(global_course_id) + "/users/", data=json.dumps(test_request_payload), headers=header
        )
    ...
    def test_get_all_course_users(self, mock_method):
        ...
        response = test_app.get("/courses/" + str(global_course_id) + "/users/", headers=header)
        response_json = response.json()
    ...
    def test_update_user(self, mock_update, mock_get):
        ...
        test_patch_payload = {"user_type": "instructor", "progress": 50.0, "approval_state": False}
        ...
        response = test_app.patch(
            "/courses/" + str(global_course_id) + "/users/" + str(global_user_id_pkey),
