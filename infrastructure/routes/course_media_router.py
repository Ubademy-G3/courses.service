...
async def create_course_media(
    payload: CourseMediaSchema, course_id: str, db: Session = Depends(get_db), apikey: str = Header(None)
):
...
async def get_all_course_media(course_id: str, db: Session = Depends(get_db), apikey: str = Header(None)):

    auth_service.check_api_key(apikey)
    course_media_list = CourseMediaController.get_all_course_media(db, course_id)
    return {"amount": len(course_media_list), "course_id": course_id, "course_media": course_media_list}


@router.get("/{media_id}", response_model=CourseMediaDB, status_code=200)
async def get_course_media(course_id: str, media_id: str, db: Session = Depends(get_db), apikey: str = Header(None)):
...
async def get_all_module_media(course_id: str, module_id: str, db: Session = Depends(get_db), apikey: str = Header(None)):

    auth_service.check_api_key(apikey)
    lis = CourseMediaController.get_all_module_media(db, module_id)
    return {"amount": len(lis), "module_id": module_id, "course_media": lis}
