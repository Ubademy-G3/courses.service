...
async def add_course_rating(
    payload: CourseRatingSchema, course_id: str, db: Session = Depends(get_db), apikey: str = Header(None)
):
...
async def get_all_course_ratings(course_id: str, db: Session = Depends(get_db), apikey: str = Header(None)):
    ...
    return {"amount": len(course_ratings), "course_id": course_id, "rating": avg, "reviews": course_ratings}
