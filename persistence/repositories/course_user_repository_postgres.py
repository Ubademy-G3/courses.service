...
class CourseUserRepositoryPostgres:
    ...
    def get_course_user(self, db, course_id, user_id):
        user = db.query(CourseUser).filter(CourseUser.course_id == course_id).filter(CourseUser.user_id == user_id).first()
        return user
    ...
    def get_course_metrics(self, db, course_id):
        ...
        if partial_query is None:
            ...
            users_currently_in_course = partial_query.filter(CourseUser.user_type == "student").filter(CourseUser.approval_state == False).count()
